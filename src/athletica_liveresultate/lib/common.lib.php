 ...
 */
     ...
     * @return        DB link
     */
    function AA_connectToDB_live()
    {          
        $db = AA_utils_connectToDB_live();
        if(!empty($GLOBALS['AA_ERROR'])) {
            AA_printErrorPage($GLOBALS['AA_ERROR']);
        }
        return $db;
	...
	*/
	function AA_checkControl(){
		...
		if(AA_checkMeetingID()){
			...
			$res = mysql_query("SELECT xControl, Online FROM athletica.meeting WHERE xMeeting = ".$_COOKIE['meeting_id']);
			if(mysql_errno() > 0){
		...
		{          			
            $sql ="SELECT 
                   ...
                   FROM 
                        ...
                        LEFT JOIN athletica.wettkampf as w ON (w.xDisziplin = d.xDisziplin)
                   WHERE 
                        w.xWettkampf = " . $event;     
	...
	if(mysql_errno() > 0){
		AA_printErrorPage(mysql_errno().": ".mysql_error());
			...
			$sql = "SELECT ss.xSerienstart, 
				 ...
				 LEFT JOIN athletica.serie AS s USING(xRunde) 
				 LEFT JOIN athletica.serienstart AS ss USING(xSerie) 
				 LEFT JOIN athletica.start AS st USING(xStart) 
				 LEFT JOIN athletica.anmeldung AS a USING(xAnmeldung) 
				 LEFT JOIN athletica.athlet AS at USING(xAthlet) 
				 LEFT JOIN athletica.wettkampf AS w ON(st.xWettkampf = w.xWettkampf) 
				 LEFT JOIN athletica.disziplin_" . $_COOKIE['language'] . " AS d USING(xDisziplin) 
				 LEFT JOIN athletica.verein AS v ON(at.xVerein = v.xVerein) 
				 LEFT JOIN athletica.rundentyp_" . $_COOKIE['language'] . " AS rt ON(r.xRundentyp = rt.xRundentyp) 
				INNER JOIN athletica.resultat AS rs ON(ss.xSerienstart = rs.xSerienstart) 
...
// get event-types categories (single, combined, club (svm))
	$res = mysql_query('SELECT Typ FROM athletica.wettkampf WHERE xMeeting = ' .$_COOKIE['meeting_id']  . ' GROUP BY Typ ORDER BY Typ');
	if(mysql_errno() > 0) {		// DB error
		AA_printErrorMsg(mysql_errno() . ": " . mysql_error());
	} else {
...
// get event-type (single, combined, club (svm))
	$res = mysql_query('SELECT 
						...
						FROM 
							athletica.runde AS r 
							LEFT JOIN athletica.wettkampf AS w USING (xWettkampf)  
...
/**
 ...
 function AA_getSortDisc($discFrom, $discTo ){
	  ...
	  $result = mysql_query("SELECT d.xDisziplin, d.Anzeige"
										. " FROM athletica.disziplin_" . $_COOKIE['language'] . " AS d" 
	...
	$eventMerged = false;    
		...
		$sql = "SELECT  
				...
				FROM
					athletica.rundenset 
				LEFT JOIN 
					athletica.runde as r USING(xRunde)
				WHERE
					xMeeting = ".$_COOKIE['meeting_id']."
				AND
					xWettkampf = ".$event;   
			...
			if ($row[0] > 0){    
				  ...
				  if($rsrow[0] > 0){   
					$sql1 = "SELECT
						  ...
						  FROM
							athletica.rundenset 
							LEFT JOIN athletica.runde USING(xRunde)
						  WHERE
							xMeeting = ".$_COOKIE['meeting_id']."
...
function AA_mergedCatEvent($category,$event){    
	...
	$sql = "SELECT  
			...
			FROM
					athletica.rundenset 
					LEFT JOIN athletica.runde as r USING(xRunde)
			WHERE
					xMeeting = ".$_COOKIE['meeting_id']."
...
function AA_getAllRoundsforChecked($event,$action,$round){   
								...
								if(mysql_errno() > 0) {
								   ...
								   if ($action=='add'){                             // add roundset 
										...
										$sqls = "SELECT 
											...
											FROM 
												athletica.runde AS r
												LEFT JOIN athletica.rundenset as rs ON  (r.xRunde=rs.xRunde AND rs.xMeeting = " .$_COOKIE['meeting_id'] .")
 ...
 function AA_addRoundset($mr,$r){
	 ...
	 if (mysql_num_rows($res_ru) == 0) {        
			...
			$result = mysql_query("SELECT MAX(xRundenset) FROM athletica.rundenset");
			$max = 0;
			if(mysql_num_rows($result) > 0){
					$row = mysql_fetch_array($result);
					$max = $row[0];
...
function AA_delRoundset($mr,$r){     
	  ...
	  if (mysql_num_rows($res_ru) > 0) { 
			...
			mysql_query("DELETE FROM athletica.rundenset WHERE
							xRundenset = $row[0]
							AND xMeeting = ".$_COOKIE['meeting_id']."
							AND xRunde = $r");
			if(mysql_errno() > 0){
				...
				$res = mysql_query("SELECT * FROM athletica.rundenset WHERE
										xRundenset = $row[0]");
				if(mysql_errno() > 0){
					$GLOBALS['AA_ERROR'] = mysql_errno().": ".mysql_error();
				}else{ 
...
function AA_checkGroup($group,$cat,$comb){	
   ...
   $qGroup="SELECT
	         ...
	         FROM
			 		athletica.wettkampf AS w
					LEFT JOIN athletica.start AS st USING(xWettkampf)
					LEFT JOIN athletica.anmeldung As a USING(xAnmeldung)
			 WHERE
...
function AA_checkRelayName($category,$event,$relayName){  
    $checkName=false;      
   	$sql="SELECT 
 			...
 			w.xDisziplin
         FROM
    	 	athletica.staffel AS st
    		LEFT JOIN athletica.start AS s ON (s.xStaffel = st.xStaffel)
    		LEFT JOIN athletica.wettkampf AS w ON (s.xWettkampf = w.xWettkampf)
  		 WHERE
  		 	st.Name = '" . $relayName ."'
...
{   
   ...
   $sql="SELECT 
            ...
            FROM 
                athletica.wettkampf as w
                LEFT JOIN athletica.meeting as m ON (m.xMeeting = w.xMeeting) 
            WHERE  
                w.xWettkampf = ". $event ." 
    ...
    // find all athletes with result -2 or -3
     $sql_d = "SELECT 
            ...
            FROM athletica.runde AS r
                ...
                LEFT JOIN athletica.resultat as re ON (re.xSerienstart = ss.xSerienstart)
       ...
       $res_d = mysql_query($sql_d);    
        ...
        foreach ($heatStarts AS $key => $val){
             ...
             if ($pass == 2) {
                 $sql = "UPDATE athletica.serienstart SET Position2 = " . $i ." WHERE xSerienstart = " . $val;  
             }
             elseif ($pass == 3) {  
                  $sql = "UPDATE athletica.serienstart SET Position3 = " . $i ." WHERE xSerienstart = " . $val;  
             }
             mysql_query($sql);    
             if(mysql_errno() > 0) {        // DB error
                    AA_printErrorMsg(mysql_errno() . ": " . mysql_error());
...
{  
    ...
    if($combined){
        ...
        $res_c = mysql_query("SELECT xRunde FROM athletica.runde WHERE xWettkampf = ".$presets['event']);
    ...
    $sql="SELECT 
                ...
                , r.Leistung                    
          FROM 
                athletica.resultat AS r
                LEFT JOIN athletica.serienstart AS ss ON ( r.xSerienstart = ss.xSerienstart )
        ...
        else
        {
            ...
            {  
                ...
                $sql_temp = "CREATE TABLE tempresult2 (
                 ...
                 $sql = "SELECT * FROM athletica.tempresult2 
                        ORDER BY
                                xSerienstart
                                , Leistung DESC";
                ...
                if(mysql_errno() > 0) {        // DB error
                    ...
                    while($row = mysql_fetch_row($result))
                    {
                        ...
                        mysql_query("
                            UPDATE athletica.serienstart SET
                                Rang = $rank
...
function AA_setMaxAthlete($xSerie, $maxAthlete){
    ...
    $sql = "Update athletica.serie Set MaxAthlet = " .$maxAthlete . " WHERE xSerie = " . $xSerie;
    mysql_query($sql);
    if(mysql_errno() > 0) {        // DB error
        AA_printErrorMsg(mysql_errno() . ": " . mysql_error());      
    }
...
function AA_resetPos($heat, $posNr){   
    ...
    $sql = "UPDATE athletica.serienstart SET " . $posName ." = 0 WHERE xSerie = " . $heat;  
    $res = mysql_query($sql);  
...
function AA_setCurrAthlete($xSerie, $xSerienstart){
    ...
    $sql = "Update athletica.serienstart Set AktivAthlet = 'n' WHERE  xSerie = " . $xSerie . " AND AktivAthlet = 'y'";
    ...
    if(mysql_errno() > 0) {        // DB error
        AA_printErrorMsg(mysql_errno() . ": " . mysql_error());      
            ...
            // set activ athlete 
            $sql = "Update athletica.serienstart Set AktivAthlet = 'y' WHERE  xSerienstart = " . $xSerienstart;    
            mysql_query($sql);
            if(mysql_errno() > 0) {        // DB error
                    AA_printErrorMsg(mysql_errno() . ": " . mysql_error());      
            }   
...
function AA_getCurrAthlete($xSerie){
    ...
    $heatStart = 0;
    $sql = "SELECT xSerienstart FROM athletica.serienstart WHERE  xSerie = " . $xSerie . " AND AktivAthlet = 'y'";
...
function AA_UpdateStatusChanged($round){
    ...
    $sql = "UPDATE athletica.runde SET StatusChanged = 'n' WHERE xRunde = " . $round;
    ...
    $sql = "UPDATE athletica.meeting SET StatusChanged = 'n' WHERE xMeeting = " .  $_COOKIE['meeting_id'];              
