...
<?php
		...
		, $wind=FALSE, $heatwind='', $time='', $svm = false, $base_perf = false, $qual_mode = false)
	{
		...
		if(!empty($title))
		{
			if(empty($heatwind))
			{    				
                $this->content .= "<h4>".$title."</h4>\r\n";
			}
			else
			{  
                $this->content .= "<h4>".$title."</h4>  (".$GLOBALS['strWind'].": ".$heatwind."\r\n";
			}
		}	// ET heat title set
        
        $this->content .= "<div class='ui-corner-all ui-shadow' style='padding: 5px;'>\r\n"; 
		...
		if($relay == FALSE)
		{    			
            $this->content .= "<th class='dialog_ctr_pda'>". $GLOBALS['strYearShort'] ."</th>
                               <th class='dialog_ctr_pda'>". $GLOBALS['strCountry'] ."</th>";
		}
		if($svm){   			
            $this->content .= "<th class='dialog_pda'>".$GLOBALS['strTeam'] ."</th>";
		}else{  			
             $this->content .= "<th class='dialog_pda'>". $GLOBALS['strClub'] ."</th>";   
		}
		
        $this->content .="<th class='dialog_ctr_pc'>".$GLOBALS['strPerformance'] ."</th>"; 
		if($wind == TRUE)
		{   			
             $this->content .= "<th class='dialog_ctr_pc'>" . $GLOBALS['strWind'] ."</th>";     
		}
		if($points == TRUE)
		{    			
             $this->content .= "<th class='dialog_ctr_pc'>". $GLOBALS['strPoints'] ."</th>";  
		}
    ...
    $this->content .="<th class='dialog_pda'>". $GLOBALS['strResultRemark'] ."</th></tr>";  
	}
	...
	{              
		...
		if($this->relay == FALSE)
		{    			
            $this->content .="<td class='forms_ctr_pda'>" . $year."</th><td class='forms_ctr_pda'>". $country ."</td>"; 
		}
		
        $this->content .="<td class='forms_pda'>" . $club ."</td><td class='forms_ctr_pc'>" . $perf ."</td>";   
		if($this->wind == TRUE)
		{      			
            $this->content .="<td class='forms_ctr_pc'>" .$wind . "</th>"; 
		}
		if($this->points == TRUE)
		{     			
            $this->content .="<td class='forms_ctr_pc'>". $points ."</th>";
		}
		...
		*/
        $this->content .="<td class='forms_pda'>" . $remark ."</td></tr>";   
		//$this->switchRowClass();
	}
