	...
	{   $this->remark = $remark; 
        ...
        if (is_null($this->remark) && $performance == ''){  
			...
			$sql_sex = "SELECT Geschlecht 
					 ...
					 LEFT JOIN athletica.wettkampf USING(xKategorie) 
					 LEFT JOIN athletica.start USING(xWettkampf) 
					 LEFT JOIN athletica.serienstart USING(xStart) 
