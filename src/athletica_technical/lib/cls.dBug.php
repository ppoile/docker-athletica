...
SCRIPTS;
	...
	* @param string $desc defines index name
	*/
	function debug($msg, $desc = NULL){
		$msg = (is_array($msg)) ? $msg : nl2br($msg);
		if(is_null($desc)){
			$this->dBugCatcher[] = $msg;
		} else {
			$this->dBugCatcher[$desc] = $msg;
		}
	}

	/**
	* writes debug messages to output
	*
	* @param boolean $bCollapsed defines if the debug array is collapsed
	*/
	function decho($bCollapsed = FALSE){
		global $glb_login;

		$this->bCollapsed = $bCollapsed;

		if(LOGGED_IN && $glb_login['SUPERADMIN']){
			...
			<div id="debug">
				<?php
				$this->checkType($this->dBugCatcher);
				?>
			</div>
			<?php
		}
	...
	/**
	* alias for the class' debug function
	*
	* @param string $msg debug message
	* @param string $desc defines index name
	*/
