function check_date(date, only_future){
	...
	if(to_return && only_future){
		...
		actual_date = actual_date.getDate()+'.'+actual_date.getMonth()+'.'+actual_date.getFullYear()+'00:00:00';
...
function check_datetime(datetime){
	...
	if(datetime){
		var pattern = new RegExp(/^[0-3]{0,1}[0-9]{1}(\.|-)(0|1){0,1}[0-9]{1}(\.|-)(19|20)[0-9]{2} [0-9]{1,2}.[0-9]{2}$/);

		if(datetime.match(pattern)){
			var datetime_array = datetime.split(' ');
			var date = datetime_array[0];
			var time = datetime_array[1];

			to_return = (check_date(date, false) && check_time(time));
		}
	}
...
function check_result(result, type){
	var to_return = false;

	var pattern = null;
	switch(type){
		case 'time':
			pattern = new RegExp(/^((((\d{1,3}:)?\d{2}|\d{1}):)?\d{2}|\d{3}|\d{1})\.\d{2}$/);
			break;
		case 'distance':
			pattern = new RegExp(/^\d{1,3}\.\d{2}$/);
			break;
		case 'score':
...
function date_to_int(date){
	...
	var hour = parseInt(date_array[3], 10);
	var minute = parseInt(date_array[4], 10);
	var second = (date_array.length==6) ? parseInt(date_array[5], 10) : 0;
	...
	to_return = parseInt(date, 10);

	return to_return;
}

function toggleDebug(target,switchToState) {
	target.style.height = (switchToState=='open') ? '' : '10px';
}
