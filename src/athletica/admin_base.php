...
if($_GET['arg'] == "reset"){
    ...
    if($errors){
        ...
        if(mysql_errno > 0){
            ...
            // reset teams
            mysql_query("DELETE FROM team WHERE xMeeting = ". $_COOKIE['meeting_id'] ."");
            if(mysql_errno > 0){
                AA_printErrorMsg(mysql_errno().": ".mysql_error());
            }else{
                // reset entries
...
document.getElementById("progress").width="150";
	...
	mysql_query("INSERT INTO base_log (type, update_time, global_last_change) VALUES ('base_$filetype','$time','$time')");     // 9.3.2018: deleted $newglc, as it is not delivered by alabus    
	if(mysql_errno() > 0){
		AA_printErrorMsg(mysql_errno().": ".mysql_error());
    }
