...
class PRINT_Page_pdf                                                       
{
  ...
  /**
	...
	{   
    $this->printTextLineBox($title,$this->font,"B",13,40,$this->lp,515,30,'left bottom',15,8);
	}
	...
	function printPageHeader(){    
        if($this->printFooter_bool) {
            ...
            if($this->landscape) {
                ...
                $this->printImage($this->picHeaderLeft,11,$this->lp,594,83,'left top');
                //$this->printImage($this->picHeaderLeft,11,$this->lp,197,24,'left top');
                //$this->printImage($this->picHeaderCenter,$this->posx,$this->lp,197,24,'center top');
                //$this->printImage($this->picHeaderRight,$this->posx,$this->lp,197,24,'right top');
                
                $this->printTextLineBox($this->stdHeaderLeft,$this->font,"B",10,11,$this->lp,197,24,'left top',0,0);
                $this->printTextLineBox($this->stdHeaderCenter,$this->font,"B",10,$this->posx,$this->lp,197,24,'center top',0,0);
                $this->printTextLineBox($this->stdHeaderRight,$this->font,"B",10,$this->posx,$this->lp,197,24,'right top',0,5);
            }
        } else {
	...
	function printPageFooter(){
        if($this->printFooter_bool) {
            if($this->landscape) {    
                ...
                $this->printImage($this->picFooterLeft,11,12,197,24,'left bottom');
                $this->printImage($this->picFooterCenter,$this->posx,12,197,24,'center bottom');
                $this->printImage($this->picFooterRight,$this->posx,12,197,24,'right bottom');
                
                $this->printTextLineBox($this->stdFooterLeft,$this->font,"B",10,11,12,197,24,'left bottom',0,0);
                $this->printTextLineBox($this->stdFooterCenter,$this->font,"B",10,$this->posx,12,197,24,'center bottom',0,0);
                $this->printTextLineBox($this->stdFooterRight,$this->font,"B",10,$this->posx,12,197,24,'right bottom',0,0);
            }
        }
	}
} // end PRINT_Page
...
class PRINT_Definitions_pdf extends PRINT_Page_pdf
{
	...
	{
		if($this->lp < 48)		// page break check; 36+12=48
		{
			$this->insertPageBreak();
		}
    $this->printTextLineBox($disc,$this->font,"",10,40,$this->lp,515,30,'left bottom',10,2);
	}
...
class PRINT_RankingList_pdf extends PRINT_Page_pdf
{
	...
	{  
		...
		// page break check
		if($this->lp < 206)		//This+169 (from HeaderLine)=37+169=206
		{ 
			$this->insertPageBreak();       
		}
		$this->lp-=22;
		$this->printTextLineBox($this->cat,$this->font,"B",13,40,$this->lp,160,30,'left bottom',13,0);
		$this->printTextLineBox($this->disc,$this->font,"B",13,$this->posx,$this->lp,160,30,'left bottom',0,0);
		$this->printTextLineBox($this->round,$this->font,"B",13,$this->posx,$this->lp,160,30,'left bottom',0,2);
	}
	...
	{ 
		if(!empty($info))
		{   
			$this->printTextLineBox($info,$this->font,"I",10,40,$this->lp,515,30,'left bottom',10,2);
		}
	}
	...
	{ 
        ...
        // page break check
		if($this->lp<169)  //Footer+8 Lines (often used)+This=36+8*12+37=169
		{  
			$this->insertPageBreak();
			$this->printSubTitle('','','','');
		}
		...
		if(!empty($this->title))
		{  
			$this->printTextLineBox($this->title,$this->font,"",11,40,$this->lp,172,30,'left bottom',11,0);
			...
			$this->printTextLineBox($time,$this->font,"",11,$this->posx,$this->lp,172,30,'right bottom',0,0);
			$this->lp-=2;
		}
        elseif ($eval == $cfgEvalType[$strEvalTypeDiscDefault]){
			if(!empty($time)){
                $this->printTextLineBox($time,$this->font,"",11,40,$this->lp,515,30,'left bottom',11,2);
            }
        }
      ...
      $ssvm=$GLOBALS['strTeamsm'];
		...
		$this->width = $this->getColWidth(507,array($GLOBALS['strRank'],28,0,0),array($stnr,$wstnr,0,0),
		...
		$this->printTextLineBox($GLOBALS['strRank'],$this->font,"B",10,40,$this->lp,$this->width[0],30,'left bottom',10,0);
		...
		$this->printTextLineBox($GLOBALS['strResultRemarkShort'],$this->font,"B",10,$this->posx+2,$this->lp,$this->width[10],30,'left bottom',0,2);
	...
	{ 
		...
		if (!$secondResult){
			// page break check
			if($this->lp < 60)		//Footer + This (2 Lines) = 36+2*12=60
			{   			   			
				$this->insertPageBreak(); 
				$this->printSubTitle('','','','');  			 	
		...
		$this->printTextLineBox($rank,$this->font,"",10,40,$this->lp,$this->width[0],30,'right bottom',10,0);
		if ($this->width[1]==0){
			$this->posx+=2;
		}else{
		...
		$this->printTextLineBox($remark,$this->font,"",10,$this->posx+2,$this->lp,$this->width[10],30,'left bottom',0,-10);
		$this->lp-=$lnqnty*12;
	}
	
	function printAthletesLine($text){ 
		$this->printTextLineBox($text,$this->font,"",8,73,$this->lp,473,30,'left bottom',10,2);
	}
...
class PRINT_CombinedRankingList_pdf extends PRINT_RankingList_pdf
{
	...
	{ 
		...
		// page break check
		if($this->lp < $add + 88)  //Footer+ 1 line + InfoLine + (time) + header  =36+12+16+(13)+24=(13)+88
		{
			$this->insertPageBreak();
		}
		$this->lp-=10;
		if(!empty($time)){
			$this->printTextLineBox($time,$this->font,"",11,40,$this->lp,515,30,'right bottom',11,2);
		}
		$this->pdf->SetFont($this->font,"B",10); 
		$this->width = $this->getColWidth(503,array($GLOBALS['strRank'],28,0,0),array($GLOBALS['strName'],200,2,0),
		...
		$this->printTextLineBox($GLOBALS['strResultRemarkShort'],$this->font,"B",10,$this->posx+2,$this->lp,$this->width[6],30,'left bottom',0,2);
		$this->pdf->SetLineWidth(0.2);
		$this->pdf->Line(40,$this->lp,555,$this->lp);
		$this->lp-=2;
	}
    ...
    function printSubTitle($category='')
    {
		...
		// page break check
        if($this->lp < 125)		//all in Header + This=101+24=125
        {        
            $this->insertPageBreak();           
        }
        $this->printTextLineBox($this->cat,$this->font,"B",13,40,$this->lp,515,30,'left bottom',22,2);
    }
	...
	{ 	$lnqnty=1;
		// page break check
		if($this->lp < 64)	//Footer + 1 Line + InfoLine = 36 + 12 + 16 = 64
		{   
			$this->insertPageBreak();
			$this->printSubTitle();
		...
		$this->printTextLineBox($rank.$pt,$this->font,"",10,40,$this->lp,$this->width[0],30,'right bottom',10,0);
		...
		$this->printTextLineBox($remark,$this->font,"",10,$this->posx+2,$this->lp,$this->width[6],30,'left bottom',0,-10);
	...
	{
       $lnqnty = $this->printTextFlowSimp($info,$this->font,"",8,80,$this->lp,400,'left',2,6);
       $this->lp -= 10*$lnqnty;   
	}
...
class PRINT_TeamRankingList_pdf extends PRINT_RankingList_pdf    //TODO
{
	...
	function printHeaderLine($time = "")
	{
		...
		// page break check
		if($this->lp < $add + 147)  //Footer + 4 lines + 4 InfoLines + (time) + header  =36+4*12+4*12+(13)+15=(13)+147
		{
			$this->insertPageBreak();
		}
		if(!empty($time)){
		...
		$this->printTextLineBox($GLOBALS['strRank'],$this->font,"B",10,40,$this->lp,$this->width[0],30,'left bottom',11,0);
		$this->printTextLineBox($GLOBALS['strName'],$this->font,"B",10,$this->posx+2,$this->lp,$this->width[1],30,'left bottom',0,0);
	...
	{
		if($this->lp < 60)	//Footer + line + infoline = 36 + 12 + 12 = 60
		{
			$this->insertPageBreak();
			$this->printSubTitle();
			$this->printHeaderLine($this->relay, $this->windinfo);
...
class PRINT_TeamSheet_pdf extends PRINT_Page_pdf
{
	...
	{
		$this->printTextLineBox($GLOBALS['strTeamRankingTitle'],$this->font,"B",11,40,$this->lp,515,30,'left bottom',20,2);
		...
		$this->printTextLineBox($GLOBALS['strPlace'] . ", " . $GLOBALS['strStadium'],$this->font,"",10,40,$this->lp,140,30,'left bottom',14,0);
		...
		$this->printTextLineBox($GLOBALS['strClub'],$this->font,"",10,40,$this->lp,140,30,'left bottom',10,0);
		...
		$this->printTextLineBox($GLOBALS['strCompetition'],$this->font,"",10,40,$this->lp,140,30,'left bottom',10,0);
	...
	{	// page break check
		if($this->lp < 98)		//Footer + 4 Lines + SubHeader = 36 + 4*12 + 14 = 98
		{
			$this->insertPageBreak();
		}
		$this->printTextLineBox($title,$this->font,"B",10,40,$this->lp,515,30,'left bottom',10,2);
		$this->pdf->SetLineWidth(0.2);
		$this->pdf->Line(40,$this->lp,555,$this->lp);
		$this->lp-=2;
	}
	...
	{	// page break check
		if($this->lp < 60)		//Footer + 2 lines = 36 + 2*12 = 60
		{
			$this->insertPageBreak();
		}
		$this->printTextLineBox($disc,$this->font,"",10,40,$this->lp,80,30,'left bottom',10,0);
		$lnqnty = $this->printTextFlowSimp($name,$this->font,"",10,$this->posx+2,$this->lp,253,'left',-10,10); //6.1: 285, >6.2: 285-30-2=253, for points
		...
		$this->printTextLineBox($total,$this->font,"",10,$this->posx+2,$this->lp,20,30,'right bottom',0,0);
	...
	{	// page break check
		...
		$lnqnty = $this->printTextFlowSimp($name,$this->font,"",10,40,$this->lp,367,'left',0,10);
	...
	{
		$this->printTextLineBox($disciplines,$this->font,"",10,50,$this->lp,505,30,'left bottom',10,2);
	}


	function printRelayAthlete($name)
	{	// page break check
		if($this->lp < 46)		//Footer + athleteline = 36 +10=46
		{
			$this->insertPageBreak();
		}
		$this->printTextLineBox($name,$this->font,"",8,130,$this->lp,425,30,'left bottom',8,2);
	}


	function printSubTotal($total)
	{	// page break check
		if($this->lp < 58)		//Footer + Subtotal = 36 + 22 = 58
		{
			$this->insertPageBreak();
		}
		$this->lp-=4;
		$this->pdf->SetLineWidth(0.2);
		$this->pdf->Line(476,$this->lp,555,$this->lp);
		$this->lp-=6;
		$this->printTextLineBox($GLOBALS['strSubTotal'],$this->font,"B",10,476,$this->lp,57,30,'left bottom',10,0);
		$this->printTextLineBox($total,$this->font,"B",10,$this->posx+2,$this->lp,20,30,'right bottom',0,2);
	}


	function printTotal($total)
	{	// page break check
		if($this->lp < 68)		//Footer + total  = 36 + 32 = 68
		{
			$this->insertPageBreak();
		}
		$this->lp-=4;
	...
	{   // page break check
		if($this->lp < 150)		//Footer + SubscriptionFooter (2 lines) + space for signature  = 36 + 2*12 + 90 = 150
		{
			$this->insertPageBreak();
		}
		$this->lp-=4;
		...
		$this->pdf->Line(40,$this->lp,555,$this->lp);
		$this->lp-=6;
		$lnqnty = $this->printTextFlowSimp($GLOBALS['strTeamRankingDeclaration'],$this->font,"",10,40,$this->lp,515,'left',0,0);
		$this->lp-=$lnqnty*12;
		$this->printTextLineBox($GLOBALS['strJudges'] . ":",$this->font,"",10,40,$this->lp,258,30,'left bottom',10,5);
...
class PRINT_Statistics_pdf extends PRINT_Page_pdf 
{
	...
	{	// page break check
		if($this->lp < 93)		//Header + 76 (from linetax) = 17 + 76 = 93
		{
			$this->insertPageBreak();
		}
	...
	{	// page break check
		if($this->lp < 64)	//Footer+Line+Total+Line=36+14+14=64
		{
			$this->insertPageBreak();
	...
	{	// page break check
		$lines=1;
		if($this->lp < 76)    //Footer + 2 Lines + Total_line=36+2*12+16=76
		{
			$this->insertPageBreak();
			$this->printHeaderLine($this->headerCol1, $this->headerCol2
