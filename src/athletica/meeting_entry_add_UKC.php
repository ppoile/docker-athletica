...
$license = 0;
if (!empty($_POST['hidden_searchfield'])){
      $license = $_POST['hidden_searchfield'];          // only for check empty fields     
}
...
if ($athlete_id > 0){
    ...
    if(!$result_a){
          ...
          if(mysql_num_rows($result_a) > 0){
               $row_ba = mysql_fetch_row($result_a);  
          }   
    }   
...
if(isset($_POST['searchfield'])){
    if(is_numeric($_POST['searchfield'])) {    // search request for license number
        ...
        $searchparam = " WHERE Lizenznummer = " . $licensenr;  
    
        $sql = "SELECT * FROM athlet " . $searchparam;    
                ...
                if(mysql_num_rows($result) == 0){ // no athlete was found
                    $search_match = false;
                      ...
                      if(mysql_num_rows($result) == 1) {
                        ...
                        $search_match = true;
                        $lastname = $row_athlete['Name'];
                        $firstname = $row_athlete['Vorname'];
                        $xAthlet = $row_athlete['xAthlet'];
                        $birth_date = $row_athlete['Geburtstag'];
                        $birth_year = $row_athlete['Jahrgang'];  
                        $nationality = $row_athlete['Land'];
                        $club = $row_athlete['xVerein'];
                        $club_name = '';                           
                        
                        $athletesex = $row_athlete['Geschlecht'];                          
                       // $catcode = $row_athlete['license_cat']; 
                        if ($row_athlete['Bezahlt'] == 'n'){
                        ...
                        //
                        $result = mysql_query("select xVerein, Name from verein where xVerein = '".$club."'");
                        if(mysql_errno() > 0){
                            AA_printErrorMsg("Line " . __LINE__ . ": ". mysql_errno() . ": " . mysql_error());
                        }else{
                        ...
                        $sql_cat = "SELECT xKategorie, Code FROM kategorie AS k WHERE k.alterslimite = " . $age . " AND k.Geschlecht = '" .$athletesex . "' AND k.UKC = 'y' AND k.aktiv = 'y'";
                                            ...
                                            if(count($_POST['combined']) > 0){
                                              ...
                                              $sql = "SELECT 
                                                    ...
                                                    WHERE 
                                                            ...
                                                            AND k.xKategorie = " .$xCat;
                                                           
                                            $res = mysql_query($sql);
                                           
                                            if (mysql_num_rows($res) >= 1) {
                                                        while($row_dis = mysql_fetch_assoc($res)){
                                                                if (!in_array($row_dis['xWettkampf'],  $xDis)) {
                                             ...
                                             // check if athlete alredy starts for this discipline(s)
                                                        foreach($xDis as $xDis1){
                                                                ...
                                                                $result = mysql_query("SELECT xStart FROM start WHERE xAnmeldung = $xAnmeldung and xWettkampf = $xDis1");
                                                                $sql="SELECT xStart FROM start WHERE xAnmeldung = $xAnmeldung and xWettkampf = $xDis";
                                                                if(mysql_errno() > 0){
            ...
            if(!$result){
                ...
                if(mysql_num_rows($result) == 0){ // no athlete was found
                    $search_match = false;
                        ...
                        $row_athlete = mysql_fetch_row($result);
                        $search_match = true;
                        $lastname = $row_athlete[0];
                        $firstname = $row_athlete[1];
                        $athlete_id = $row_athlete[2];
                        $birth_date = $row_athlete[3];
                        $nationality = $row_athlete[4];
                        $athletesex =  $row_athlete[5];   
...
{   
    ...
    $_POST['address'] = (isset($_POST['address'])) ? $_POST['address'] : $_POST['address_hidden'];
    $_POST['plz'] = (isset($_POST['plz'])) ? $_POST['plz'] : $_POST['plz_hidden']; 
    $_POST['city'] = (isset($_POST['city'])) ? $_POST['city'] : $_POST['city_hidden'];   
    $_POST['email'] = (isset($_POST['email'])) ? $_POST['email'] : $_POST['email_hidden'];               
   
    $gebDate = sprintf("%04d-%02d-%02d", $_POST['year'], '00', '00');     
       ...
       ?>
        <script type="text/javascript">  
            ...
            if (athReg) {
                ...
                if (check == false){     
                    window.open('meeting_entry_add_UKC.php', 'detail');  
                   }
            }
   ...
   if($club == 0){
    ...
    //if((empty($_POST['name']) || empty($_POST['first']) || empty($_POST['club']) || empty($_POST['address']) || empty($_POST['plz']) || empty($_POST['city']) || empty($_POST['email']) || $licInvalid )
    //    && ($athlete_id == 0 && $license == 0) ) // if athlete id is given, athlete exists in base data
 ...
 //   elseif (!is_numeric($_POST['plz']) && $license == 0) {
  //          AA_printErrorMsg($strErrPlzField);   
  //  }
    elseif ( !empty($_POST['email']) && !check_email($_POST['email']) && $license == 0){ 
                   AA_printErrorMsg($strErrEmailField);
                   $focus_email = 1;          
    }  
    ...
    {   
        ...
        {
            AA_printErrorMsg($strErrAgeNotUKC);
            $focus_year = 1;                
        }
        else
        {
                        ...
                        else    // check startnbr
                        {
                            ...
                            if($nbrcheck == TRUE)    // Valid startnbr available
                            {
                                ...
                                //{
                                    $xAthlet = 0;
                                    $licnr = 0;
                                    if ($_POST['startnbr'] > 0 && $startnbr == 0) {
                                        $startnbr  = $_POST['startnbr'];
                                    }
                                    
                                     if ($license > 0){
                                         
                                           // athlete with license 
                                           $sql = "SELECT
                                                xAthlet
                                            FROM
                                                athlet
                                    ...
                                    {   
                                        // add from manually entered data
                                            // check athleticagen
                                            //$agen = 'y';
                                            ...
                                            $sql = "INSERT INTO athlet SET 
                                                    Name=\"" . $lastname . "\"                                                      
                                                    , Vorname=\"" . $firstname . "\"
                                                    , Geburtstag=\"" . $_POST['year'] . "-01-01"  . "\"
                                                    , Jahrgang='" . $_POST['year'] . "'
                                                    , xVerein=" . $_POST['club']."                                                         
                                                    , Geschlecht = '".$athletesex."'
                                                    , Lizenztyp = 3   
        ...
        if($event_row[3] == $cfgEventType[$strEventTypeSingleCombined]){
            ...
            if($comb != $event_row[10]){
               ...
               global $arr_checkedCombs;
                ...
                if ($val == '' || $ukc_meeting == 'n') { //dont overwrite value ... no guess what can be stored in $val before.. ***SBA***
                    $sql = "
                        SELECT 
                            season_effort, notification_effort 
                        FROM
                ...
                <td class='dialog-top' nowrap="nowrap" id="topperftd<?php echo $combCat.$comb ?>">
                    <input type="checkbox" value="<?php echo $event_row[8]."_".$comb ?>" name="combined[]" id="combinedCheck<?php echo $event_row[8]; ?>_<?php echo $comb; ?>" 
                        onclick="check_combined('<?php echo $event_row[8]."_".$comb ?>', this);
                            validate_discipline(<?php echo "'" . $ukc_meeting . "','" .$combCat.$comb."', '".$event_row[11]."', ".$event_row[12] ?>,this.checked);"<?php echo $checked; ?>>
                    ...
                    <div id="div_<?php echo $event_row[8]; ?>_<?php echo $comb; ?>" style="position: relative; display: none;">
                        ...
                        if($checked==' checked="checked"'){
                            ...
                            <script type="text/javascript">
                                ...
                                validate_discipline('<?php echo "'" . $ukc_meeting . "','" . $combCat; ?><?php echo $comb; ?>', '<?php echo $event_row[11]; ?>', <?php echo $event_row[12]; ?>,this.checked);
                            </script>
                            <?php
                        }
                        ?>
            ...
            <td class="dialog-top" nowrap="nowrap" id='topperftd<?php echo $event_row[2]?>'>
                ...
                <input name='events[]' type='checkbox' value='<?php echo $event_row['2'] ?>'
                 onclick="validate_discipline(<?php echo $ukc_meeting .',\'' . $event_row[2] .',\'' . $event_row[11].'\',' . $event_row[12].','?>this.checked)" <?php echo $checked; ?>/><?php echo $event_row[0] .$info?></td>
            <?php
...
$page = new GUI_Page('meeting_entry_add_UKC',false,'stylesheet_small-fonts.css');
$page->startPage();
...
?>
    ...
    function check_category(asfb){ 
       ...
       // gMonth = document.getElementById('newmonth').value;
        ...
        if (typeof(asfb) == 'boolean') {
           document.location.href='meeting_entry_add_UKC.php?argument=change_sex&asfb='+asfb+'&name='+gName+'&firstname='+gFirstname+'&year='+gYear+'&sex='+gSex+'&country='+gCountry+'&region='+gRegion+'&clubtext='+gClubText+'&club='+gClub+'&clubnr='+gClubNr+'&clubinfo='+gClubInfo+'&club2='+gClub2+'&startnbr='+gStartnbr+'&category='+gCategory+'&team='+gTeam+'&combined='+gCombined+gDisc+gComb+'&licNr='+gLicNr+'&licPrinted='+gLicPrinted+'&athleteId='+gAthleteId+'&licType='+gLizType;                                                                                                                                                                                
        }  
        else {
            document.location.href='meeting_entry_add_UKC.php?argument=change_sex&name='+gName+'&firstname='+gFirstname+'&year='+gYear+'&sex='+gSex+'&country='+gCountry+'&region='+gRegion+'&clubtext='+gClubText+'&club='+gClub+'&clubnr='+gClubNr+'&clubinfo='+gClubInfo+'&club2='+gClub2+'&startnbr='+gStartnbr+'&category='+gCategory+'&team='+gTeam+'&combined='+gCombined+gDisc+gComb+'&licNr='+gLicNr+'&licPrinted='+gLicPrinted+'&athleteId='+gAthleteId+'&licType='+gLizType;                                                                                                                                                                                                                                                                         
        }
    ...
    function change_asfb(asfb){
        ...
        document.location.href='meeting_entry_add_UKC.php?asfb='+asfb+'&name='+gName+'&firstname='+gFirstname+'&year='+gYear+'&sex='+gSex+'&country='+gCountry+'&region='+gRegion+'&club='+gClub+'&clubinfo='+gClubInfo+'&club2='+gClub2+'&startnbr='+gStartnbr+'&category='+gCategory+'&team='+gTeam+'&combined='+gCombined+gDisc+gComb;
    ...
    function change_licType(o){
        ...
        document.location.href = 'meeting_entry_add_UKC.php?licType='+o.value+'&name='+gName+'&firstname='+gFirstname+'&year='+gYear+'&sex='+gSex+'&country='+gCountry+'&region='+gRegion+'&club='+gClub+'&clubinfo='+gClubInfo+'&club2='+gClub2+'&startnbr='+gStartnbr+'&category='+gCategory+'&team='+gTeam+'&combined='+gCombined+gDisc+gComb;
    ...
    function validate_discipline(ukc, disc, sex, limit, o){   
...
<tr>                   
    ...
    <td class='forms' colspan="2">
        ...
        $menu->addSearchfield('meeting_entry_add_UKC.php', '_self', 'post', '', false);
        $menu->printMenu();
        ?>
    </td>    
</tr>
...
<tr> 
     ...
     if ( (isset($_POST['searchfield']) && (!is_numeric($_POST['searchfield'])) ) ) { 
         ...
         <td class="forms">    
                  <form action='meeting_entry_add_UKC.php' method='post' name='athleteSearch'>  
                  <input name='arg' type='hidden' value='change_athlete' />         
         <?php  
        
         $dropdown = new GUI_Select('athleteSearch', 1, "document.athleteSearch.submit()");                
        
         $sql_athlets = "SELECT Name, Vorname, Lizenznummer FROM athlet " . $searchparam;     
...
<table>
<form action='meeting_entry_add_UKC.php' method='post' name='selectcat'>
<input name='search' type='hidden' value='<?php echo $_POST['searchfield']; ?>' />  
</form>
<form action='meeting_entry_add_UKC.php' method='post' name='entry'>
    <tr>
        <td class='forms'>
            <button type='submit' >
                <?php echo $strEnter; ?>
...
<tr>
    ...
    <th class='dialog'><?php echo $strKt ?></td>
    <?php $dd = new GUI_RegionDropDown($region, "", false, 'y'); ?>
</tr>
<?php
...
<tr>
    ...
    ?>
    <th class='dialog'><?php echo $strYear; ?> (YYYY)</th>
    <td class='forms' colspan="1">
    ...
    <input type="hidden" name="address_hidden" id="newaddress_hidden" value="">
    <input type="hidden" name="plz_hidden" id="newplz_hidden" value="">
    <input type="hidden" name="clubtext_hidden" id="clubtext_hidden" value="">
    <input type="hidden" name="city_hidden" id="newcity_hidden" value="">
    <input type="hidden" name="email_hidden" id="newemail_hidden" value="">
    ...
    <input name='year' type='text'
        ...
        id="newyear"  size="4" onchange="check_year()">
...
<tr>
    ...
    <td class='forms' ><input class='text' name='address' type='text'
        maxlength='25' value='<?php echo $address; ?>'/></td>            
    <th class='dialog'><?php echo $strKt ?></td>
    <?php $dd = new GUI_RegionDropDown($region, "", false, 'y'); ?>
</tr>
     <?php
   ...
   <tr>
    ...
    <input name='plz' type='text'
       ...
       size="4" >
        ...
        maxlength='25' value='<?php echo $city; ?>'/></td>            
...
<tr>
    ...
    <td class='forms'><input type="text"  name="email"
         size="30" value="<?php echo $email;?>"></td>
</tr>
...
<!--
    ...
    var focus_email = '<?php echo $focus_email; ?>';
     var focus_year = '<?php echo $focus_year; ?>';  
        ...
        if (focus_license==1){ 
            if (document.lookup){  
             ...
             if(document.entry ) {  
                ...
                else if (document.entry.year.value=='' || focus_year == 1) {  
                    document.entry.year.focus();
                } 
                else if (document.entry.clubtext.value=='') {  
                    document.entry.clubtext.focus();
                } 
                //else if (document.entry.address.value=='') {  
               //     document.entry.address.focus();
               // } 
               // else if (document.entry.plz.value=='' || (isNaN(document.entry.plz.value) == true)) {  
               //     document.entry.plz.focus();
               // } 
                //else if (document.entry.city.value=='') {  
                //    document.entry.city.focus();
               // } 
               // else if (document.entry.email.value=='' || focus_email == 1 ) {  
