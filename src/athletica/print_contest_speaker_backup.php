...
else
{
    ...
    if($round > 0)
    {
        $where_final = ($_GET['final'] == 1) ? " AND ss.Rang <= 8" : "";
        $order2 = ($_GET['final'] == 1) ? "ss.Rang ASC" : "ss.Position";
        // display all heats
        ...
        else
        {
            ...
            case($cfgDisciplineType[$strDiscTypeDistance]):
                ...
                if(mysql_num_rows($res_sl)==0){
                    ...
                    $sl_result = ($timepices[0] * 360 * 1000) + ($timepices[1] * 60 * 1000) + ($timepices[2] *  1000) + ($timepices[3]);
                    if($layout == $cfgDisciplineType[$strDiscTypeTrack] || $layout == $cfgDisciplineType[$strDiscTypeTrackNoWind]) {
                        $sl_result = AA_formatResultTime($sl_result, true, true);
                    } else {
                        $sl_result = AA_formatResultTime($sl_result,true, false);
                    }                             
                    $sl_name = $row_sl['sl_firstname']." ".$row_sl['sl_lastname']." (".date("d.m.",strtotime($row_sl['sl_date'])).")";
                }
                ...
                if(!$relay) {
                    ...
                    while($row = mysql_fetch_row($result))
                    {
                        ...
                        {                      
                            ...
                            $doc->printRecords($sr_result, $sr_name, $sl_result, $sl_name);
                        ...
                        if($relay == FALSE) {
                            ...
                            $doc->printHeatLine($row[12], $row[6], "$row[8] ".strtoupper($row[7])
                                    , AA_formatYearOfBirth($row[9]), $row[10], $row[4], $row[14],$season_effort,$best_effort, $palmares, $res_prev_text);
                        }
                        else
                        {    
            ...
            case($cfgDisciplineType[$strDiscTypeNone]):
                ...
                if(mysql_num_rows($res_sl)==0){
                    ...
                    $row_sl = mysql_fetch_array($res_sl);
                    $sl_result = AA_formatResultMeter(str_replace(".", "", $row_sl['sl_result']));                                                          
                    $sl_name = $row_sl['sl_firstname']." ".$row_sl['sl_lastname']." (".date("d.m.",strtotime($row_sl['sl_date'])).")";
                }
                ...
                while($row = mysql_fetch_row($result))
                {
                   ...
                   //else if($b > $maxLines)
                    ...
                    $doc->printHeatLine($row[6], "$row[8] ".strtoupper($row[7])
                            , AA_formatYearOfBirth($row[9]), $row[10], $row[14],$season_effort,$best_effort, $row[4], $palmares, $result_qual);
                }
                break;
            }        // end switch "Layout"
